 ...
 * @notice In this adjusted version we take a whitelist of addresses into account that will not incur a fee
 */
contract FeeCalculator is Ownable {
    ...
    */
     ...
     * @param _recipient - recipeint address of the tip, as public goods don't incur a fee
    ...
    function _checkPG(address grantee) public view returns (bool) {
        return publicGoods[grantee];
    }

    function addPublicGood(address adminAddress) external onlyOwner {
        publicGoods[adminAddress] = true;
    }

    function deletePublicGood(address adminAddress) external onlyOwner {
        delete publicGoods[adminAddress];
    }
     ...
     * @param _recipient - recipient address, as public goods don't incur a fee
     * @return fee - processing fee, few percent of slippage is allowed
    ...
    function _splitPayment(uint256 _valueToSplit, AssetType _assetType, address _recipient) internal view returns (uint256 fee, uint256 value) {
        ...
        uint256 paymentFee = getPaymentFee(_valueToSplit, _assetType, _recipient);
